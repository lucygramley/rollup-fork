...
function arrayExpression (node: ArrayExpression, position, buffer) {
    const {scope} = node;node.elements = convertNodeList(node, scope, buffer[position], buffer)
},
...
function arrowFunctionExpression (node: ArrowFunctionExpression, position, buffer) {
    const {scope} = node;const flags = buffer[position];
node.async = (flags & 1) === 1;node.expression = (flags & 2) === 2;node.generator = (flags & 4) === 4;const annotations = (node.annotations = convertAnnotations(buffer[position + 1], buffer));
...
function assignmentExpression (node: AssignmentExpression, position, buffer) {
    const {scope} = node;node.operator = FIXED_STRINGS[buffer[position]] as estree.AssignmentOperator;
...
function assignmentPattern (node: AssignmentPattern, position, buffer) {
    const {scope} = node;node.left = convertNode(node, scope, buffer[position], buffer);
node.right = convertNode(node, scope, buffer[position + 1], buffer);
},
function awaitExpression (node: AwaitExpression, position, buffer) {
    const {scope} = node;node.argument = convertNode(node, scope, buffer[position], buffer);
},
function binaryExpression (node: BinaryExpression, position, buffer) {
    const {scope} = node;node.operator = FIXED_STRINGS[buffer[position]] as estree.BinaryOperator;
node.left = convertNode(node, scope, buffer[position + 1], buffer);
node.right = convertNode(node, scope, buffer[position + 2], buffer);
},
function blockStatement (node: BlockStatement, position, buffer) {
    const {scope} = node;node.body = convertNodeList(node, scope, buffer[position], buffer)
},
function breakStatement (node: BreakStatement, position, buffer) {
    const {scope} = node;const labelPosition = buffer[position];
...
function callExpression (node: CallExpression, position, buffer) {
    const {scope} = node;const flags = buffer[position];
node.optional = (flags & 1) === 1;node.annotations = convertAnnotations(buffer[position + 1], buffer)
...
function catchClause (node: CatchClause, position, buffer) {
    const {scope} = node;const paramPosition = buffer[position];const parameter = (
node.param = paramPosition === 0 ? null : convertNode(node, scope, paramPosition, buffer));
...
function chainExpression (node: ChainExpression, position, buffer) {
    const {scope} = node;node.expression = convertNode(node, scope, buffer[position], buffer);
},
function classBody (node: ClassBody, position, buffer) {
    const {scope} = node; const bodyPosition = buffer[position];
			  if (bodyPosition) {
          ...
          for (let index = 0; index < length; index++) {
            ...
            body[index] = convertNode(
                node,
                buffer[nodePosition] !== 79 && (buffer[nodePosition + 3] & /* the static flag is always first */ 1) === 0 ? scope.instanceScope : scope,
                nodePosition,
                buffer
            );
          }
...
function classDeclaration (node: ClassDeclaration, position, buffer) {
    const {scope} = node;node.decorators = convertNodeList(node, scope, buffer[position], buffer)
const idPosition = buffer[position + 1];
node.id = idPosition === 0 ? null : convertNode(node, scope.parent as ChildScope, idPosition, buffer)
const superClassPosition = buffer[position + 2];
node.superClass = superClassPosition === 0 ? null : convertNode(node, scope, superClassPosition, buffer)
...
function conditionalExpression (node: ConditionalExpression, position, buffer) {
    const {scope} = node;node.test = convertNode(node, scope, buffer[position], buffer);
...
function directive (node: ExpressionStatement, position, buffer) {
    const {scope} = node;node.directive = buffer.convertString(buffer[position]);
...
function doWhileStatement (node: DoWhileStatement, position, buffer) {
    const {scope} = node;node.body = convertNode(node, scope, buffer[position], buffer);
...
function exportAllDeclaration (node: ExportAllDeclaration, position, buffer) {
    const {scope} = node;const exportedPosition = buffer[position];
node.exported = exportedPosition === 0 ? null : convertNode(node, scope, exportedPosition, buffer)
...
function exportDefaultDeclaration (node: ExportDefaultDeclaration, position, buffer) {
    const {scope} = node;node.declaration = convertNode(node, scope, buffer[position], buffer);
},
function exportNamedDeclaration (node: ExportNamedDeclaration, position, buffer) {
    const {scope} = node;node.specifiers = convertNodeList(node, scope, buffer[position], buffer)
...
node.declaration = declarationPosition === 0 ? null : convertNode(node, scope, declarationPosition, buffer)
},
function exportSpecifier (node: ExportSpecifier, position, buffer) {
    const {scope} = node;node.local = convertNode(node, scope, buffer[position], buffer);
const exportedPosition = buffer[position + 1];
node.exported = exportedPosition === 0 ? node.local : convertNode(node, scope, exportedPosition, buffer);
},
...
function forOfStatement (node: ForOfStatement, position, buffer) {
    const {scope} = node;const flags = buffer[position];
node.await = (flags & 1) === 1;node.left = convertNode(node, scope, buffer[position + 1], buffer);
node.right = convertNode(node, scope, buffer[position + 2], buffer);
node.body = convertNode(node, scope, buffer[position + 3], buffer);
},
function forStatement (node: ForStatement, position, buffer) {
    const {scope} = node;const initPosition = buffer[position];
...
function functionDeclaration (node: FunctionDeclaration, position, buffer) {
    const {scope} = node;const flags = buffer[position];
node.async = (flags & 1) === 1;node.generator = (flags & 2) === 2;const annotations = (node.annotations = convertAnnotations(buffer[position + 1], buffer));
node.annotationNoSideEffects = annotations.some(comment => comment.type === 'noSideEffects')
...
node.alternate = alternatePosition === 0 ? null : convertNode(node, (node.alternateScope = new TrackingScope(scope)), alternatePosition, buffer)
},
function importAttribute (node: ImportAttribute, position, buffer) {
    const {scope} = node;node.key = convertNode(node, scope, buffer[position], buffer);
...
function importExpression (node: ImportExpression, position, buffer) {
    const {scope} = node;node.source = convertNode(node, scope, buffer[position], buffer);
...
function importSpecifier (node: ImportSpecifier, position, buffer) {
    const {scope} = node;const importedPosition = buffer[position];
node.local = convertNode(node, scope, buffer[position + 1], buffer);
node.imported = importedPosition === 0 ? node.local : convertNode(node, scope, importedPosition, buffer);
},
function jsxAttribute (node: JSXAttribute, position, buffer) {
    const {scope} = node;node.name = convertNode(node, scope, buffer[position], buffer);
...
function jsxElement (node: JSXElement, position, buffer) {
    const {scope} = node;node.openingElement = convertNode(node, scope, buffer[position], buffer);
node.children = convertNodeList(node, scope, buffer[position + 1], buffer)
const closingElementPosition = buffer[position + 2];
node.closingElement = closingElementPosition === 0 ? null : convertNode(node, scope, closingElementPosition, buffer)
},
...
function jsxFragment (node: JSXFragment, position, buffer) {
    const {scope} = node;node.openingFragment = convertNode(node, scope, buffer[position], buffer);
node.children = convertNodeList(node, scope, buffer[position + 1], buffer)
...
function jsxMemberExpression (node: JSXMemberExpression, position, buffer) {
    const {scope} = node;node.object = convertNode(node, scope, buffer[position], buffer);
...
function jsxNamespacedName (node: JSXNamespacedName, position, buffer) {
    const {scope} = node;node.namespace = convertNode(node, scope, buffer[position], buffer);
...
function jsxOpeningElement (node: JSXOpeningElement, position, buffer) {
    const {scope} = node;const flags = buffer[position];
node.selfClosing = (flags & 1) === 1;node.name = convertNode(node, scope, buffer[position + 1], buffer);
node.attributes = convertNodeList(node, scope, buffer[position + 2], buffer)
},
...
function labeledStatement (node: LabeledStatement, position, buffer) {
    const {scope} = node;node.label = convertNode(node, scope, buffer[position], buffer);
...
const value = node.value = (flags & 1) === 1;node.raw = value ? "true" : "false";
},
...
function logicalExpression (node: LogicalExpression, position, buffer) {
    const {scope} = node;node.operator = FIXED_STRINGS[buffer[position]] as estree.LogicalOperator;
node.left = convertNode(node, scope, buffer[position + 1], buffer);
node.right = convertNode(node, scope, buffer[position + 2], buffer);
},
function memberExpression (node: MemberExpression, position, buffer) {
    const {scope} = node;const flags = buffer[position];
node.computed = (flags & 1) === 1;node.optional = (flags & 2) === 2;node.object = convertNode(node, scope, buffer[position + 1], buffer);
node.property = convertNode(node, scope, buffer[position + 2], buffer);
},
function metaProperty (node: MetaProperty, position, buffer) {
    const {scope} = node;node.meta = convertNode(node, scope, buffer[position], buffer);
node.property = convertNode(node, scope, buffer[position + 1], buffer);
},
function methodDefinition (node: MethodDefinition, position, buffer) {
    const {scope} = node;const flags = buffer[position];
node.static = (flags & 1) === 1;node.computed = (flags & 2) === 2;node.decorators = convertNodeList(node, scope, buffer[position + 1], buffer)
...
function newExpression (node: NewExpression, position, buffer) {
    const {scope} = node;node.annotations = convertAnnotations(buffer[position], buffer)
...
function objectExpression (node: ObjectExpression, position, buffer) {
    const {scope} = node;node.properties = convertNodeList(node, scope, buffer[position], buffer)
},
...
function property (node: Property, position, buffer) {
    const {scope} = node;const flags = buffer[position];
node.method = (flags & 1) === 1;node.shorthand = (flags & 2) === 2;node.computed = (flags & 4) === 4;const keyPosition = buffer[position + 1];
...
function returnStatement (node: ReturnStatement, position, buffer) {
    const {scope} = node;const argumentPosition = buffer[position];
node.argument = argumentPosition === 0 ? null : convertNode(node, scope, argumentPosition, buffer)
},
function sequenceExpression (node: SequenceExpression, position, buffer) {
    const {scope} = node;node.expressions = convertNodeList(node, scope, buffer[position], buffer)
},
...
function switchCase (node: SwitchCase, position, buffer) {
    const {scope} = node;const testPosition = buffer[position];
node.test = testPosition === 0 ? null : convertNode(node, scope, testPosition, buffer)
...
function switchStatement (node: SwitchStatement, position, buffer) {
    const {scope} = node;node.discriminant = convertNode(node, node.parentScope, buffer[position], buffer);
...
function taggedTemplateExpression (node: TaggedTemplateExpression, position, buffer) {
    const {scope} = node;node.tag = convertNode(node, scope, buffer[position], buffer);
node.quasi = convertNode(node, scope, buffer[position + 1], buffer);
},
function templateElement (node: TemplateElement, position, buffer) {
    const flags = buffer[position];
node.tail = (flags & 1) === 1;const cookedPosition = buffer[position + 1];
...
function templateLiteral (node: TemplateLiteral, position, buffer) {
    const {scope} = node;node.quasis = convertNodeList(node, scope, buffer[position], buffer)
...
function tryStatement (node: TryStatement, position, buffer) {
    const {scope} = node;node.block = convertNode(node, scope, buffer[position], buffer);
...
node.finalizer = finalizerPosition === 0 ? null : convertNode(node, scope, finalizerPosition, buffer)
},
function unaryExpression (node: UnaryExpression, position, buffer) {
    const {scope} = node;node.operator = FIXED_STRINGS[buffer[position]] as estree.UnaryOperator;
...
function updateExpression (node: UpdateExpression, position, buffer) {
    const {scope} = node;const flags = buffer[position];
node.prefix = (flags & 1) === 1;node.operator = FIXED_STRINGS[buffer[position + 1]] as estree.UpdateOperator;
...
function variableDeclaration (node: VariableDeclaration, position, buffer) {
    const {scope} = node;node.kind = FIXED_STRINGS[buffer[position]] as estree.VariableDeclaration['kind'];
...
function variableDeclarator (node: VariableDeclarator, position, buffer) {
    const {scope} = node;node.id = convertNode(node, scope, buffer[position], buffer);
...
function yieldExpression (node: YieldExpression, position, buffer) {
    const {scope} = node;const flags = buffer[position];
node.delegate = (flags & 1) === 1;const argumentPosition = buffer[position + 1];
node.argument = argumentPosition === 0 ? null : convertNode(node, scope, argumentPosition, buffer)
}
];
